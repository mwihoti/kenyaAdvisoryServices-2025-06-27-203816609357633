...
const Agribot = () => {
  ...
  const sendMessage = async (messages) => {
    try {
      ...
      const messagesToSend = messages.slice(1).filter(msg => 
        'user' in msg || ('system' in msg && msg.system.content !== "Analyzing your agricultural question...")
      );
      
      // Convert to backend format - the backend expects LLM.ChatMessage format
      const backendMessages = messagesToSend.map(msg => {
        if ('user' in msg) {
          return { user: { content: msg.user.content } };
        } else {
          return { system_: { content: msg.system.content } };
        }
      });
      
      const response = await backend.chat(backendMessages);
